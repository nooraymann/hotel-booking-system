...
Connection conn = DriverManager.getConnection(url, "root", "webproject");
	...
	String check=request.getParameter("avilible");
	String updatesql="update rooms set Available=?"+"where ID="+ID;
	PreparedStatement updatecheck=conn.prepareStatement(updatesql);

	updatecheck.setString(1,check);
